...
//        return joinBytes(output[0], output[1], output[2], output[3]);
//    }
//
//    /* 将32比特数拆分成4个8比特数 */
...
//
//    /* 将4个8比特数合并成32比特数 */
//    private static int joinBytes(byte byte_0, byte byte_1, byte byte_2, byte byte_3) {
//        return ((byte_0 & 0xFF) << 24) | ((byte_1 & 0xFF) << 16) | ((byte_2 & 0xFF) << 8) | (byte_3 & 0xFF);
//    }
//
//
//    /* 密钥拓展 */
//    public static int[] keyGenerate(byte[] key) {
...
//        //将输入的密钥每32比特合并，并异或FK
//        for (int i = 0; i < 4; i++) {
//            key_temp[i] = joinBytes(key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]);
